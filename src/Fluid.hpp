...
#define ROWS ((float)(WIDTH) / (SCALE))
#define COLUMNS ((float)(HEIGHT) / (SCALE))
...
class Fluid {
  ...
  const int IX(int x, int y) const {
    // NOTE: Make sure x and y don't use off the screen values

    x = x < 0 ? 0 : x;
    x = x > COLUMNS - 1 ? COLUMNS - 1 : x;
  ...
  void project(float velocX[], float velocY[], float p[], float div[]) {
    ...
    for (int j = 1; j < ROWS - 1; j++) {
      for (int i = 1; i < COLUMNS - 1; i++) {
        velocX[IX(i, j)] -= 0.5f * (p[IX(i + 1, j)] - p[IX(i - 1, j)]) * ROWS;
        velocY[IX(i, j)] -= 0.5f * (p[IX(i, j + 1)] - p[IX(i, j - 1)]) * COLUMNS;
      }
    }
    set_bnd(1, velocX);
    set_bnd(2, velocY);
